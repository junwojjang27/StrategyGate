...
import kr.ispark.common.system.service.ExcelHeaderVO;
import kr.ispark.common.system.service.ExcelVO;
import kr.ispark.common.util.CommonUtil;


public class BscExcelDownloadView extends AbstractExcelView {
	...
	private HSSFSheet makeSheet(HSSFWorkbook workbook, HashMap excelMap) throws Exception{
		...
		data.setAlignment (data.getAlignmentEnum().LEFT);
		dataRight.setAlignment (dataRight.getAlignmentEnum().RIGHT);
		dataCenter.setAlignment (dataCenter.getAlignmentEnum().CENTER);

		titleStyle.setVerticalAlignment(titleStyle.getVerticalAlignmentEnum().CENTER);
		titleStyle2.setVerticalAlignment(titleStyle.getVerticalAlignmentEnum().CENTER);
		headerStyle.setVerticalAlignment(headerStyle.getVerticalAlignmentEnum().CENTER);
		data.setVerticalAlignment(data.getVerticalAlignmentEnum().CENTER);
		dataRight.setVerticalAlignment(dataRight.getVerticalAlignmentEnum().CENTER);
		dataCenter.setVerticalAlignment(dataCenter.getVerticalAlignmentEnum().CENTER);

		/**********************************
		 * Cell 테두리 선긋기
		 **********************************/
		headerStyle.setBorderTop(headerStyle.getBorderTopEnum().THIN);
		headerStyle.setBorderBottom(headerStyle.getBorderBottomEnum().THIN);
		headerStyle.setBorderLeft(headerStyle.getBorderLeftEnum().THIN);
		headerStyle.setBorderRight(headerStyle.getBorderRightEnum().THIN);

		data.setBorderBottom(data.getBorderBottomEnum().THIN);
		data.setBorderLeft(data.getBorderLeftEnum().THIN);
		data.setBorderRight(data.getBorderRightEnum().THIN);
		data.setBorderTop(data.getBorderTopEnum().THIN);

		dataRight.setBorderBottom(data.getBorderBottomEnum().THIN);
		dataRight.setBorderLeft(data.getBorderLeftEnum().THIN);
		dataRight.setBorderRight(data.getBorderRightEnum().THIN);
		dataRight.setBorderTop(data.getBorderTopEnum().THIN);

		dataCenter.setBorderBottom(data.getBorderBottomEnum().THIN);
		dataCenter.setBorderLeft(data.getBorderLeftEnum().THIN);
		dataCenter.setBorderRight(data.getBorderRightEnum().THIN);
		dataCenter.setBorderTop(data.getBorderTopEnum().THIN);
		...
		if(excelTitle != null && !"".equals(excelTitle)) {
			...
			cell.setCellStyle(titleStyle);
			sheet.addMergedRegion(new CellRangeAddress(0, 0, 0, excelInfoList.size()-1));
		}
		...
		if(null != excelSearchInfoList && 0 < excelSearchInfoList.size()) {
			for(int i = 0; i < excelSearchInfoList.size(); i++) {
				...
				cell = row.getCell((short)0);   //0셀 불러오기
				cell.setCellValue("▣" + excelVO.getSearchTitle() + " : " + CommonUtil.removeNull(excelVO.getSearchValue(),""));
				cell.setCellStyle(titleStyle2);
				//sheet.addMergedRegion(new CellRangeAddress(0, 0, 0, excelInfoList.size()-1));
			}
		}
		...
		if(excelHeaderInfoList != null && excelHeaderInfoList.size() > 0){
			...
			//머징
			for(ExcelHeaderVO vo :  excelHeaderInfoList){
				sheet.addMergedRegion(new CellRangeAddress(index+vo.getStartY()-1, index+vo.getEndY()-1, vo.getStartX()-1, vo.getEndX()-1));
			}
					...
					if(!excelVO.isRowspan()){
						if(excelVO.getRowspanColumn() != null && !"".equals(excelVO.getRowspanColumn())){
							...
							if(rowspanObj != null) {
								...
								if(colRowCnt[j] == 0 && (index+rowspanCount-1) > index) {
									sheet.addMergedRegion(new CellRangeAddress(index, index+rowspanCount-1, j, j));
								}
						...
						if(excelVO.getRowspanColumn() != null && !"".equals(excelVO.getRowspanColumn())){
							...
							if(rowspanValue.equals(prevValue)){
								...
								if(colCnt>0){
									//셀병합
									sheet.addMergedRegion(new CellRangeAddress(prevIndex, prevIndex+colCnt, j, j));
								}else{
									//이전로우에 대한 셀병합이 필요없다.
								}
								//카운팅0
