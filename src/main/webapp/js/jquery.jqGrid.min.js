...
loadui:"enable",toolbar:[!1,""],scroll:!1,multiboxonly:!1,deselectAfterSort:!0,scrollrows:!1,autowidth:!1,scrollOffset:18,cellLayout:5,subGridWidth:20,multiselectWidth:70,gridview:!1,rownumWidth:25,rownumbers:!1,pagerpos:"center",recordpos:"right",footerrow:!1,userDataOnFooter:!1,hoverrows:!0,altclass:"ui-priority-secondary",viewsortcols:[!1,"vertical",!0],resizeclass:"",autoencode:!1,remapColumns:[],ajaxGridOptions:{},direction:"ltr",toppager:!1,headertitles:!1,scrollTimeout:40,data:[],_index:{},
...
b.jgrid.stripHtml(f)+'"':""));B=B+(' aria-describedby="'+a.p.id+"_"+A+'"');return B+t[0]},r=function(c){return c==null||c===""?"&#160;":a.p.autoencode?b.jgrid.htmlEncode(c):""+c},
u=function(c,d,e,f,g){var h=a.p.colModel[e];if(h.formatter!==void 0){c=""+a.p.idPrefix!==""?b.jgrid.stripPref(a.p.idPrefix,c):c;c={rowId:c,colModel:h,gid:a.p.id,pos:e};d=b.isFunction(h.formatter)?h.formatter.call(a,d,c,f,g):b.fmatter?b.fn.fmatter.call(a,h.formatter,d,c,f,g):r(d)}
else d=escapeHTML(r(d));	// 20171214 kimyh formatter가 없는 경우 html escape 처리
...
this).hover(function(){b(this).addClass("ui-state-hover")},function(){b(this).removeClass("ui-state-hover")});if(this.p.multiselect){var ha=[],Y;b("#cb_"+b.jgrid.jqID(a.p.id),this).bind("click",function(){a.p.selarrrow=[];var c=a.p.frozenColumns===true?a.p.id+"_frozen":"";if(this.checked){b(a.rows).each(function(d){
		if(d>0&&!b(this).hasClass("ui-subgrid")&&!b(this).hasClass("jqgroup")&&!b(this).hasClass("ui-state-disabled")){
			if(b("#jqg_"+b.jgrid.jqID(a.p.id)+"_"+b.jgrid.jqID(this.id)).is(":visible")) {	// 화면에 보이는 체크박스만 선택되도록 조건문 추가
		...
		}});Y=true;ha=[]}else{b(a.rows).each(function(d){if(d>0&&!b(this).hasClass("ui-subgrid")&&!b(this).hasClass("ui-state-disabled")){b("#jqg_"+b.jgrid.jqID(a.p.id)+"_"+b.jgrid.jqID(this.id))[a.p.useProp?
...
d,L,L);P++;if(this.p.datatype==="local"){H[this.p.localReader.id]=V;this.p._index[V]=this.p.data.length;this.p.data.push(H);H={}}}this.p.altRows===true&&!u&&(c==="last"?(this.rows.length-1)%2===1&&b(this.rows[this.rows.length-1]).addClass(T):b(this.rows).each(function(a){a%2===1?b(this).addClass(T):b(this).removeClass(T)}));
this.updatepager(true,true);a=true;
// 20180206 kimyh 데이터 없음 메시지 삭제
b(this).find(".noGridResult").remove();
}));return a},footerData:function(d,g,c){function e(a){for(var b in a)if(a.hasOwnProperty(b))return!1;return!0}var a,i=!1,f={},h;void 0===d&&
...
f));for(;e.length-1-g<h;)e+="0"}}if(b.thousandsSeparator){h=b.thousandsSeparator;g=e.lastIndexOf(f);g=-1<g?g:e.length;var f=e.substring(g),j=-1,i;for(i=g;0<i;i--){j++;if(0===j%3&&i!==g&&(!d||1<i))f=h+f;f=e.charAt(i-1)+f}e=f}
e=b.prefix?b.prefix+e:e;
e=b.suffix?e+b.suffix:e;
// 20171208 kimyh 소수점 이하 값이 0이면 제거
e = (b.removePointZeros) ? e.replace(/\.0+$/, "").replace(/(\.\d*[1-9])0*/, "$1") : e;
return e
}return c}};a.fn.fmatter.defaultFormat=function(c,b){return a.fmatter.isValue(c)&&""!==c?c:b.defaultValue||"&#160;"};a.fn.fmatter.email=function(c,b){return!a.fmatter.isEmpty(c)?'<a href="mailto:'+c+'">'+c+"</a>":a.fn.fmatter.defaultFormat(c,
...
f,""];if(!0===e.time&&!(!1===c&&a.jgrid.isEmpty(d))&&!a.jgrid.checkTime(d))return[!1,h+": "+a.jgrid.edit.msg.date+" - hh:mm (am/pm)",""];if(!0===e.url&&!(!1===c&&a.jgrid.isEmpty(d))&&(g=/^(((https?)|(ftp)):\/\/([\-\w]+\.)+\w{2,3}(\/[%\-\w]+(\.\w{2,})?)*(([\w\-\.\?\\\/+@&#;`~=%!]*)(\.\w{2,})?)*\/?)/i,!g.test(d)))return[!1,h+": "+a.jgrid.edit.msg.url,""];
if(!0===e.custom&&!(!1===c&&a.jgrid.isEmpty(d)))return a.isFunction(e.custom_func)?(
	// 20171213 kimyh editrules의 custom_func의 arguments에 colmodel 추가
	d=e.custom_func.call(this,d,h,b,f[b]),a.isArray(d)?d:[!1,a.jgrid.edit.msg.customarray,""]
...
n=b.jgrid.createEl.call(c,i.edittype,j,e,!0,b.extend({},b.jgrid.ajaxOptions,c.p.ajaxSelectOptions||{}));b(h).html("").append(n).attr("tabindex","0");b.jgrid.bindEv.call(c,n,j);
//window.setTimeout(function(){b(n).focus()},0);
/*
 * 20180330 kimyh
 * cell edit시 input text인 경우 select() 처리
 */
window.setTimeout(function () {
	if(b(n)[0].tagName.toUpperCase() == "INPUT" && b(n).attr("type") == "text") {
		b(n).select();
	} else {
		b(n).focus();
	}
},0);
b("input, select, textarea",h).bind("keydown",function(a){a.keyCode===27&&(b("input.hasDatepicker",h).length>0?b(".ui-datepicker").is(":hidden")?b(c).jqGrid("restoreCell",d,f):b("input.hasDatepicker",h).datepicker("hide"):b(c).jqGrid("restoreCell",d,f));
// 20171205 kimyh textarea에서는 enter키로 줄바꿈을 할 수 있도록 처리
if(a.keyCode===13 && (a.target.tagName === undefined || a.target.tagName != 'TEXTAREA')){b(c).jqGrid("saveCell",d,f);return false}
...
.bind("blur",function(e) {
	...
	if(b("input.hasDatepicker",h).length == 0) {
...
gridResize:function(a){return this.each(function(){var d=this,c=b.jgrid.jqID(d.p.id);if(d.grid&&b.fn.resizable){a=b.extend({},a||{});if(a.alsoResize){a._alsoResize_=a.alsoResize;delete a.alsoResize}else a._alsoResize_=
