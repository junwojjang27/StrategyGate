	...
	return this.each( function() {
		...
		formatter = function (rowId, cellval , colpos, rwdat, _act){
			...
			if(cm.formatter !== undefined) {
				...
				//v = escapeHTML(cellVal(cellval));	// 20171214 kimyh formatter가 없는 경우 html escape 처리
				v = cellVal(cellval);	// 20171214 kimyh formatter가 없는 경우 html escape 처리
			}
			return v;
		},
					...
					$(ts.rows).each(function(i) {
						if (i>0) {
							if(!$(this).hasClass("ui-subgrid") && !$(this).hasClass("jqgroup") && !$(this).hasClass('ui-state-disabled')){
								if($("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(this.id)).is(':visible')) { //visible true인 경우에만 실행되도록 if문 추가(2012.7.10 하윤식)
	...
	addRowData : function(rowid,rdata,pos,src) {
		...
		if(rdata) {
			...
			this.each(function() {
				...
				// 20180206 kimyh 데이터 없음 메시지 삭제
				$(t).find(".noGridResult").remove();
			});
		}
		return success;
	},
	...
	$.fmatter.util = {
		...
		NumberFormat : function(nData,opts) {
			...
			if($.fmatter.isNumber(nData)) {
				...
				// 20171208 kimyh 소수점 이하 값이 0이면 제거
				sOutput = (opts.removePointZeros) ? sOutput.replace(/\.0+$/, "").replace(/(\.\d*[1-9])0*/, "$1") : sOutput;
			...
			if(edtrul.custom === true) {
				if( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
					if($.isFunction(edtrul.custom_func)) {
						// 20171213 kimyh editrules의 custom_func의 arguments에 colmodel 추가
						var ret = edtrul.custom_func.call(g,val,nm,valref,cm[valref]);
	...
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				/*
				 * 20180330 kimyh
				 * cell edit시 input text인 경우 select() 처리
				 */
				window.setTimeout(function () {
					if($(elc)[0].tagName.toUpperCase() == "INPUT" && $(elc).attr("type") == "text") {
						$(elc).select();
					} else {
						$(elc).focus();
					}
				...
				$("input, select, textarea",cc).bind("keydown",function(e) {
					...
					if (e.keyCode === 13 && (e.target.tagName === undefined || e.target.tagName != 'TEXTAREA')) {	// 20171205 kimyh textarea에서는 enter키로 줄바꿈을 할 수 있도록 처리
				...
				.bind("blur",function(e) {
					// 20171205 kimyh 날짜 입력란이 아닌 경우 blur시 입력값 저장
					if($("input.hasDatepicker",cc).length == 0) {
						$($t).jqGrid("saveCell",iRow,iCol);
					}
				});
...
$.jgrid.extend({
	...
	scrollToBottom:function() {	// 20171211 kimyh 하단 스크롤 함수 추가
		var $gridBody = $(this).parents(".ui-jqgrid-bdiv:first");
		$gridBody.scrollTop($(this).height());
	}
});
})(jQuery);
/*
