...
$(function(){
	$("#list").jqGrid({
		url			:	"${context_path}/system/system/code/codeGrp/codeGrpList_json.do",
		postData	:	getFormData("form"),
		width		:	"${jqgrid_width}",
		height		:	"250",
		colModel	:	[
						{name:"codeGrpId",	index:"codeGrpId",	width:80,	align:"center",	label:"<spring:message code="word.codeGroupId"/>"},
						...
						*/
						...
						{name:"modifyYn",		index:"modifyYn",		width:80,	align:"center",	label:"<spring:message code="word.modifyYn"/>",
						...
						{name:"codeDefId",	index:"codeDefId",	width:100,	align:"center",	label:"<spring:message code="word.codeDefId"/>",
							editable:true, edittype:"select", formatter:'select', editroles:{required:true},
							editoptions:{value:getCodeDefSelect()}
						},
						{name:"yearYn",		index:"yearYn",		width:100,	align:"center",	label:"<spring:message code="word.yearlyManageYn"/>",
							editable:true, edittype:"select", formatter:"select", editroles:{required:true},
						...
						{name:"codeCnt",	index:"codeCnt",	width:80,	align:"center",	label:"<spring:message code="word.codeCnt"/>",
	...
	$("#listChild").jqGrid({
		url			:	"${context_path}/system/system/code/codeGrp/codeList_json.do",
		postData	:	getFormData("form"),
		width		:	"${jqgrid_width}",
		height		:	"250",
		colModel	:	[
...
function getMultiCodeValue(elem, operation, value){
	...
	var gridId = id = $elems.eq(1).attr("id").split('_')[1];
	var dataIds = $("#listChild").jqGrid("getDataIDs");

	var id = dataIds[Number(gridId)-1];

	$("#listChild #"+id).find("td[aria-describedby='listChild_codeNmLang']").text(bindResultStr);
	$("#listChild #"+id).find("td[aria-describedby='listChild_codeNmLang']").attr("title",bindResultStr);
	*/
...
function getCodeDefSelect(){
	var selectStr="";
	<c:forEach var="useYnList" items="${codeUtil:getCodeList('023')}" varStatus="status">
...
function searchList() {
 	$("#codeGrpId").val("");
 	$("#yearYn").val("");
	loadPage("${context_path}/system/system/code/codeGrp/codeGrpList.do", "form");
}
...
function saveData() {
	...
	if(gridToFormChanged("list", "form", true)){
		sendAjax({
			"url" : "${context_path}/system/system/code/codeGrp/saveCodeGrp.do",
...
function doDeleteData() {
	...
	sendAjax({
		"url" : "${context_path}/system/system/code/codeGrp/deleteCodeGrp.do",
		"data" : getFormData("form"),
		"doneCallbackFunc" : "checkResult"
	});
}
...
function saveCodeData() {
	...
	if(gridToForm("listChild", "form", true)){
		sendAjax({
			"url" : "${context_path}/system/system/code/codeGrp/saveCode.do",
			"data" : getFormData("form"),
...
// 삭제 처리
	...
	sendAjax({
		"url" : "${context_path}/system/system/code/codeGrp/deleteCode.do",
		"data" : getFormData("form"),
		"doneCallbackFunc" : "searchSaveCodeList"
	});
}
...
function getModifySelect() {
	var selectStr="";
	<c:forEach var="modYnList" items="${codeUtil:getCodeList('366')}" varStatus="status">
		selectStr += "<c:out value="${modYnList.codeId}"/>"+":"+"<c:out value="${modYnList.codeNm}"/>";
		<c:if test="${not status.last}">
			selectStr += ";";
		</c:if>
	</c:forEach>
