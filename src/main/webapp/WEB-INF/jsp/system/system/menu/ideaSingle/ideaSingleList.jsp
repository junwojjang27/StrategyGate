...
<script type="text/javascript">
    $(function () {
        $("#list").jqGrid({
            ...
            colModel: [
                ...
                {name: "state", 	index: "state", 	width: 30, align: "center", 	label: "<spring:message code="word.progressStatus"/>"},
				...
				{name: "content", 	index: "content", 	width: 30, hidden: true},
        ...
        if (cnt != 1) {		//관리자 아닐 경우
            ...
            for (var n = 1; n <= num; n++) {	//1 ~ (화면에 표시되고있는 레코드 숫자) 까지 반복하여 ideaCd를 비교해서 해당 제안을 찾음.
                ...
                if (rowData.ideaCd == $("#ideaCd").val()) {
                    if (rowData.userId == userId && rowData.state == '대기') {
                        $(".save").show();
                    } else {
                        $(".save").hide();
                        ischange = false;
    ...
    function setDetail(data) {
        ...
        voToForm(dataVO, "form", ["title", "year", "category", "content"]);	//VO의 값을 form에 세팅
    ...
    function addData() {
        ...
        $("#newForm").show();
        resetForm("form", ["category", "title", "content", "ideaCd"]);		//form에 있는 값 지우기
    ...
    <div class="sch-bx">
        <ul>
            <li>
                ...
                <form:select path="findYear" class="select wx100" items="${codeUtil:getCodeList('017')}"
    ...
    <div id="newForm">
        ...
        <div class="tbl-type02">
            <table summary="">
                ...
                <tbody>
                <tr>
                    <th scope="row"><label for="category"><spring:message code="word.category"/></label><span
                            class="red">(*)</span></th>
                    <td>
                        <form:select path="category" class="select wx100" items="${codeUtil:getCodeList('385')}"
                                     itemLabel="codeNm" itemValue="codeId">
                        </form:select>
                    </td>
                </tr>
                <tr>
                    <th scope="row"><label for="title"><spring:message code="word.title"/></label><span
                            class="red">(*)</span></th>
                    <td><form:input path="title" class="t-box01" maxlength="300" id="title1"/></td> <!--최대길이 지정-->
                </tr>
                <tr>
                    <th scope="row"><label for="content"><spring:message code="word.content"/></label><span class="red">(*)</span>
                    </th>
                    <td>
