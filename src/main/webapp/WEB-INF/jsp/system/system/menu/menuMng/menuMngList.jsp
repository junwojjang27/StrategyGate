...
$(function(){
	$("#list").jqGrid({
		url			:	"${context_path}/system/system/menu/menuMng/menuMngList_json.do",
		...
		colModel	:	[
						...
						*/
						{name:"pgmNm",	index:"pgmNm",	width:200,	align:"left",	label:"<spring:message code="word.menu"/>",
						...
						{name:"add",	    index:"add", 		width:40, align:"center", hidden:false, formatter:addUnderRow, label:"하위<br />추가"},
						...
						{name:"pgmGbnId",			index:"pgmGbnId",				width:100,	align:"center",		label:"<spring:message code="word.division"/>",
							editable:true, edittype:"select", formatter:'select', editrules:{required:true}, editoptions:{value:getGbnSelect()}
						},
						...
						{name:"param",		index:"param",		width:80,	align:"left", hidden:true,	label:"<spring:message code="word.menuParam"/>",
							editable:true, edittype:"text", editrules:{custom:true, custom_func:jqGridChkBytes},	editoptions:{maxlength:150}
						},	
...
function popRoleInfo(rowId){
	...
	openFancybox({
		"url" : "${context_path}/system/system/menu/menuMng/popRoleList.do",
		"data" : data
	});
}

function multiElement(value, options){
    
	var dataIds = options.id.split("_");
	var dataId = dataIds[0];
	
	var gridIds = $("#list").jqGrid("getDataIDs");
	var id = gridIds[Number(dataId)-1];
	
	var idVal = $("#list tr#"+id).find("td[aria-describedby='list_authGubuns']").text();
	
	var val = {};
	//if(value != "" && -1<value.indexOf(",")){val = value.split(",");}else{val=new Array(1); val[0]=value;}
	if(idVal != ""){val = idVal.split(",");}else{val=new Array(1); val[0]=value;}
	var selected;
    ...
    var multiSelect = "<select multiple style='height:150px;'>";
	      for(var i=0 ; i<array.length ; i++){
	    	  items = array[i].split(":");
	    	  selected = "";
	    	  for(var j=0 ; j<val.length ; j++){
	    		  if(items[0] == val[j]){selected="selected";break;}
	    	  }
	    	  multiSelect += "<option value=\""+items[0]+"\" "+selected+">"+items[1]+"</option>";
	      }
	multiSelect += "</select>";
...
function multiVal(elem, operation, value){
	...
	if(operation === 'get') {
		...
		var el = $(elem);
		el.children("option:selected").each(function(i,v){
			returnTitle += $(this).text()+",";
		});
	...
	$("#list tr#"+id).find("td[aria-describedby='list_authGubuns']").text(returnVal);
	$("#list tr#"+id).find("td[aria-describedby='list_authGubuns']").attr("title",returnVal);
...
function addUnderRow(cellvalue, options, rowObject){
	var str = "";
	if(rowObject.pgmId != ''){
		str = "<a href='#' onclick='addRow(\"" + removeNull(rowObject.pgmId) + "\",\"" + removeNull(rowObject.pgmLevelId) + "\",\"" + removeNull(rowObject.pgmNm) + "\",\"" + removeNull(options.rowId) + "\");return false;'>" + "+" + "</a>";
	}
	return str;
}
...
function saveData() {
	...
	sendAjax({
		"url" : "${context_path}/system/system/menu/menuMng/saveMenuMng.do",
...
function popGuideComment(rowId) {
	...
	openFancybox({
		"url" : "${context_path}/system/system/menu/menuMng/popGuideComment.do",
		"data" : getFormData("form")
	});
