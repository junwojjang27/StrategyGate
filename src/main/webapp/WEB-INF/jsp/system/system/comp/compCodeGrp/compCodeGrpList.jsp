...
$(function(){
	$("#list").jqGrid({
		url			:	"${context_path}/system/system/comp/compCodeGrp/compCodeGrpList_json.do",
	...
	$("#listChild").jqGrid({
		url			:	"${context_path}/system/system/comp/compCodeGrp/compCodeList_json.do",
		postData	:	getFormData("form"),
		width		:	"${jqgrid_width}",
		height		:	"250",
		colModel	:	[
...
function searchList() {
 	$("#codeGrpId").val("");
 	$("#yearYn").val("");
	loadPage("${context_path}/system/system/comp/compCodeGrp/compCodeGrpList.do", "form");
}
...
function saveData() {
	...
	if(gridToFormChanged("list", "form", true)){
		sendAjax({
			"url" : "${context_path}/system/system/comp/compCodeGrp/saveCompCodeGrp.do",
...
function doDeleteData() {
	...
	sendAjax({
		"url" : "${context_path}/system/system/comp/compCodeGrp/deletecCompCodeGrp.do",
		"data" : getFormData("form"),
		"doneCallbackFunc" : "checkResult"
	});
}
...
function saveCodeData() {
	...
	if(gridToForm("listChild", "form", true)){
		sendAjax({
			"url" : "${context_path}/system/system/comp/compCodeGrp/saveCompCode.do",
			"data" : getFormData("form"),
...
// 삭제 처리
	...
	sendAjax({
		"url" : "${context_path}/system/system/comp/compCodeGrp/deleteCompCode.do",
		"data" : getFormData("form"),
	...
	<div class="sch-bx">
		<ul>
			...
			<li>
				...
				<form:select path="findUseYn" class="select wx80" >
					<option value=""><spring:message code="word.all" /></option>
					<form:options items="${codeUtil:getCodeList('011')}" itemLabel="codeNm" itemValue="codeId"></form:options>
				</form:select>
			</li>
		</ul>
