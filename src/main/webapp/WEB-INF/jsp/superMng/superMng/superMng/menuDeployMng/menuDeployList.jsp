...
$(function(){
	...
	$("#list").jqGrid({
		url			:	"${context_path}/superMng/superMng/superMng/menuDeployMng/menuDeployList_json.do",
						...
						*/
						...
						{name:"authGubunNms",	index:"authGubunNms", width:120, label:"<spring:message code="word.auth"/>",
							formatter:function(cellvalue, options, rowObject){
								return "<div style='width:90%;text-overflow:ellipsis;white-space:nowrap;overflow:hidden;float:left;'>"+removeNull(cellvalue)+"</div>" + "<a href='#' onclick='popRoleInfo(\"" + options.rowId + "\");return false;'><img src='${img_path}/icon_search.gif'/></a>";
							},
...
function popRoleInfo(rowId){
	...
	openFancybox({
		"url" : "${context_path}/superMng/superMng/superMng/menuDeployMng/popRoleList.do",
		"data" : data
	});
}
...
function addRow(pgmId,pgmLevelId,pgmNm,rowId){
	...
	if(pgmId != ''){
		...
		$("#"+newId).find("td > a:first").attr("onclick","delRow(\""+newId+"\")").text("-");
	}
}
...
function saveData() {
	...
	sendAjax({
		"url" : "${context_path}/superMng/superMng/superMng/menuDeployMng/saveMenuDeploy.do",
